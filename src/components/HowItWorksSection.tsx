      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {steps.map((s, i) => (
            ...
            >
              <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-2xl gradient-bg shadow-xl shadow-primary/20">
                <s.icon className="h-9 w-9 text-primary-foreground" />
              </div>
              <span className="mb-2 block font-display text-sm font-bold tracking-widest text-primary">
                STEP {s.step}
              </span>
              ...
              {i < steps.length - 1 && (
                <div className="absolute right-0 top-10 hidden translate-x-1/2 md:block">
                  <div className="h-px w-16 gradient-bg opacity-40" />
                </div>
              )}
            </motion.div>
          ))}
